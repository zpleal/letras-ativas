...
window.addEventListener("load", () => {
  ...
  fetch("panels.js").then(response => response.json()).then(configs => populateImagesPanel(configs)).catch(console.log);

  $('openConfigs').onclick = () => configsDialog.style.display = 'block';

  $('ok').onclick = () => {
    doFullScreen(); // gamePanel.imageURL = getRadioValue('panel');
...
window.addEventListener('resize', () => {
  if (gamePanel) gamePanel.readjust();
});
...
function doFullScreen() {
  ...
  } else console.log('fullscreen disabled');
}
/*
function doFullScreen() {
    const fullscreen = getRadioValue('fullscreen');
...
function saveSelections() {
  ...
  for (const name in DEFAULT_SELECTIONS.radios) selections.radios[name] = getRadioValue(name);

  for (const id in DEFAULT_SELECTIONS.values) selections.values[id] = $(id).value;
...
function restoreSelections() {
  ...
  for (const name in DEFAULT_SELECTIONS.radios) setRadioValue(name, selections.radios[name]);

  for (const id in DEFAULT_SELECTIONS.values) $(id).value = selections.values[id];
}
...
function populateOn(parent, panelStruct) {
  for (const label in panelStruct) {
    ...
    if (isLeaf) element.value = value;else populateOn(element, value);
  }
}
/*
